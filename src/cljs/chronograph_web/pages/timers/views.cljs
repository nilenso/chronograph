...
(defn- start-stop-button [{:keys [id] :as timer}]
  (if (running? timer)
...
(defn- delete-button [timer-id]
  (r/with-let [show-delete-popconfirm? (r/atom false)]
    [antd/popconfirm {:title           "Are you sure you want to delete this timer?"
                      ...
                      :cancelText      "No"
                      :placement       "bottom"
                      :onConfirm       #(rf/dispatch [::timers-events/delete-timer timer-id])}
     [antd/button {:type    "danger"
                   :icon    icons/DeleteOutlined
                   :onClick #(reset! show-delete-popconfirm? true)}]]))

(defn- task-name-row [task-name]
  [antd/row [antd/text {:class  "large-text"
                        :strong "true"} task-name]])
...
(defn timer-display [{:keys [task note id] :as timer}]
  [:<>
   [antd/row {:class  (str "timer " (when (running? timer)
    ...
    [antd/col
     [task-name-row (:name task)]
     [note-row note]
     [antd/row
      [antd/space
       [start-stop-button timer]
       [edit-timer-button-and-form id]
       [delete-button id]]]]]])
...
(defn create-timer-widget []
  (r/with-let [{::form/keys [get-submit-attributes
                             get-input-attributes
                             get-select-attributes]}
               (form/form {:form-key        ::create-timer-form
                           :specs           {:task :tasks/id}
    ...
    [antd/space {:direction "vertical"
     ...
     [antd/space
      [antd/button (assoc (get-submit-attributes) :icon icons/PlayCircleFilled)
       "Start"]
      [antd/button
       {:onClick #(rf/dispatch [::timers-events/dismiss-create-timer-widget])}
       "Cancel"]]]))
