...
(rf/reg-event-fx
  ::reject-invite
  (fn [{:keys [db]} [_ id]]
    {:http-xhrio (http/post {:uri        (str "/api/invitations/" (pending-invites-db/slug-by-id db id) "/reject")
...
(rf/reg-event-fx
  ::accept-invite
  (fn [{:keys [db]} [_ id]]
    {:http-xhrio (http/post {:uri (str "/api/invitations/" (pending-invites-db/slug-by-id db id) "/accept")
