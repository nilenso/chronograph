      ...
      (is (every? (partial s/valid? :timers/timer)
                  (into (repeatedly 3 (fn [] (timer/create! db/datasource
                        ...
                        (repeatedly 3 (fn [] (timer/create! db/datasource
                                                            ...
                                                            "A sample note.")))))
          "lets user-2 create multiple timers for multiple tasks.")
      (let [timer (timer/create! db/datasource
                                 organization-id
                                 user-2-id
                                 task-id
...
(deftest delete-timer-isolation-by-user-test
  (let [user-1-id (:user-1-id @test-context)
        user-2-id (:user-2-id @test-context)
        task-id (:task-id-1 @test-context)
        organization-id (:organization-id @test-context)]
    (testing "Users trying to delete each others' timers."
      (let [timer-by-user-1 (timer/create! db/datasource
                                          organization-id
                                          user-1-id
                                          task-id
                                          "A note by user-1.")
...
(deftest update-note-isolation-by-user-test
  (let [user-1-id (:user-1-id @test-context)
    ...
    (testing "Users trying to update each others' timer notes."
      (let [unstarted-timer-by-user-1 (timer/create! db/datasource
                                                    organization-id
                                                    user-1-id
                                                    task-id
                                                    "A note by user-1.")
...
(deftest stop-timer-isolation-by-user-test
  (let [user-1-id (:user-1-id @test-context)
    ...
    (testing "Users trying to stop each others' timers."
      (let [running-timer-by-user-1 (let [timer (timer/create! db/datasource
                                                              organization-id
                                                              user-1-id
                                                              task-id
                                                              "A note by user-1.")]
    ...
    (let [user-2-id (:user-2-id @test-context)
      ...
      (dotimes [_ 3]
        (->> (timer/create! db/datasource
             ...
             :time-spans/timer-id
             (timer/stop! db/datasource user-2-id)
             :time-spans/timer-id
             (timer/start! db/datasource user-2-id)))
...
(deftest find-timer-isolation-by-user-test
  (let [user-1-id (:user-1-id @test-context)
    ...
    (testing "Users trying to read each others' timers."
      (let [timer-for-task-timed-by-user-1 (timer/create! db/datasource
                                                         organization-id
                                                         user-1-id
